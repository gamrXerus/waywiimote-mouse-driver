...
static void xdg_output_logical_position(void* data, zxdg_output_v1* output, int32_t x, int32_t y) {
    // Not used
}
...
static void registry_global(void* data, wl_registry* registry, uint32_t name, const char* interface, uint32_t version) {
    ...
    if (strcmp(interface, zxdg_output_manager_v1_interface.name) == 0) {
        state->output_manager = (zxdg_output_manager_v1*)wl_registry_bind(registry, name, &zxdg_output_manager_v1_interface, 2);
    } else if (strcmp(interface, wl_output_interface.name) == 0) {
        state->output = (wl_output*)wl_registry_bind(registry, name, &wl_output_interface, 1);
    }
}
...
void getScreenResolution(int& width, int& height) {
    ...
    if (!state.output_manager) {
        std::cerr << "Wayland compositor does not support zxdg_output_manager_v1." << std::endl;
        width = 1920; // Fallback
        height = 1080;
    ...
    zxdg_output_v1* output = zxdg_output_manager_v1_get_xdg_output(state.output_manager, state.output);
    zxdg_output_v1_add_listener(output, &xdg_output_listener, &state);
...
private:
    ...
    void internalSetScreenArea(
        ...
        screenAreaTopLeft = Vector3f(
            min(left, right).toFloat(),
            min(top, bottom).toFloat(),
            0L
        );
        screenAreaBottomRight = Vector3f(
            max(left, right).toFloat(),
            max(top, bottom).toFloat(),
            0L
        );
        computeMouseMat();
    }
...
public:
    ...
    WiiMouse(Xwiimote::Ptr wiimote) : wiimote(wiimote), vmouse(10001) {
        ...
        calmatY = Vector3(0, Scalar(10000, 768), 0).redivide(100);
